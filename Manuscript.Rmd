...
com@data$Elevation <- raster::extract(Env$DEM, as(com, "SpatialPolygons"), df = T, fun = mean)[,2]
Deriv <- DEMderiv(raster(com, 'Elevation'), c('slope', 'curvature', 'plancurvature', 'profcurvature', 'cosaspect'), path = '/usr/bin/')
names(Deriv)[5] <- 'SW'
com@data <- cbind(com@data, raster::extract(Deriv, as(com, "SpatialPolygons"), df = T, fun = mean)[-1])
com@data$Wetness <- raster::extract(DEMderiv(Env$DEM, 'wetness', path = '/usr/bin/'),
                                    as(com, "SpatialPolygons"), df = T, fun = mean)[,2]
com@data$Canopy <- raster::extract(Env$Canopy, as(com, "SpatialPolygons"), df = T, fun = mean)[,2]
...
R <- rownames_to_column(R, "community")
save(R, file = file.path("draft_save", 'R2.Rdata'))
```
...
load(file.path("draft_save", 'R2.Rdata'))
# GWM
GWM <- trees %>% 
  left_join(PFT) %>% 
  filter(!is.na(strata)) %>% 
...
trees %>% 
  left_join(PFT) %>% 
  filter(!is.na(strata)) %>% 
  dplyr::group_by(strata, community, trait) %>% 
  summarise(GWM = weighted.mean(traitvalue, Abundance, na.rm = T))
```
...
trees %>% 
  group_by(community, SpCode) %>% 
  summarise(Abundance = sum(Abundance)) %>% 
  left_join(select(PFT,SpCode, strata)) %>% 
  unique() %>% 
  filter(!is.na(strata)) %>% 
  group_by(community, strata) %>% 
  summarise(N = sum(Abundance), S = length(unique(SpCode))) %>% 
  group_by(strata) %>% 
  # summarise(N = sd(N), S = sd(S))
  summarise(N = mean(N), Nsd = sd(N),
            S = mean(S), Ssd = sd(S))
  # group_by(strata, community) %>% summarise(N = sum(Abundance), S = length(unique(SpCode))) %>% group_by(strata) %>% summarise(N = mean(N), S = mean(S))
```
...
data <- genPFT() %>% 
  ...
  mutate(SLA = log(SLA)) %>% 
  select(-CE) %>% 
  mutate(LMA = 1/SLA, LT.LDMC = LT*LDMC) %>% 
  group_by(Tree, Sp, SpCode) %>% 
  summarise_all(mean, na.rm = T)
summary(lm(LMA ~ LT.LDMC, data))
ggplot(data, aes(LT.LDMC, LMA)) +
  geom_point() +
  geom_smooth(method = "lm") +
  ggtitle("LMA = 0.30 + 0.17 LT x LDMC",
          "r2 = 0.48; n = 267; P < 0.001") +
  xlab("LT x LDMC")
...
lng <- 75.662778
lat <- 12.537500
bbox <- c(left = lng-d, bottom = lat-d, right = lng+d, top = lat+d)
...
g <- ggplot() +  
  ...
  coord_equal() +
  theme(axis.line = element_blank(), legend.position = c(0.9, 0.5), axis.text = element_text(face = "bold"),
  ...
  ggsn::north(symbol = 10, x.min = min(DCM$x) + 5, x.max = max(DCM$x), y.min = min(DCM$y) + 30, y.max = max(DCM$y)-10) +
  xlab("Longitude") + ylab("Latitude") +
  scale_x_continuous(expand=c(0,0)) + scale_y_continuous(expand=c(0,0)) +
  annotation_custom(grob = ggplotGrob(
    ggmap(map) +
      geom_point(x = lng, y = lat, colour = "red", size = 4) +
      theme(axis.title = element_blank(), axis.line = element_blank(), 
            axis.ticks = element_blank(), axis.text = element_blank(),
            panel.border = element_rect(fill = NA))
  ),
  xmin = min(DCM$x), xmax = min(DCM$x) + 200,
  ymin = max(DCM$y) - 200, ymax = max(DCM$y))
# ggsave(file = "Plot.png", path = './data/', width = 150, height = 200, unit = 'mm', dpi = 300)
ggsave(g, filename = "figures/Figure_1.tiff", device = "tiff", dpi = 600, bg = "white", width = 8, height = 8, compression = "lzw")
```
...
# spectrum.boxplots <- pca.plot$data %>%
  ...
  # rename(`Axis 1` = Comp.1, `Axis 2` = Comp.2) %>%
...
g <- autoplot(PFT.pca, 
  ...
  geom_hline(aes(yintercept = 0), col = 'green', linetype = "dotted") +
  geom_vline(aes(xintercept = 0), col = 'purple', linetype = "dotted") +
  stat_ellipse(aes(group = strata, col = strata), linetype = "dashed") +
  xlab('Axis 1') + ylab('Axis 2') +
  theme(legend.position = c(1, 1), 
        legend.justification = c(1, 1),
        panel.border = element_rect(colour = "black", fill=NA, size=1)) +
  scale_color_discrete("Guild")
ggsave(g, filename = "figures/Figure_2.tiff", device = "tiff", dpi = 600, bg = "white", width = 8, height = 8, compression = "lzw")
```
...
g <- GWM %>% 
  mutate(trait = recode(trait, 
                        ...
                        "LA" = "D",
                        "WD" = "E")) %>% 
  mutate(trait = factor(trait, levels = c("A", "B", "C", "D", "E"))) %>%
  ggplot(aes(x = strata, y = GWM, fill = strata)) +
  geom_boxplot() +
  geom_text(aes(label = label), label.size = 0.2,
            data.frame(
              strata = "C",
              trait = LETTERS[1:5],
              label = c("A SLA", "B LDMC", "C LT", "D LA", "E WD"),
              GWM = c(2.8, 435, 28, 10, 0.7)
            )) +
  facet_wrap(~trait, scales = "free", nrow = 1,
             strip.position = "left", 
             labeller = as_labeller(c(A = "SLA (m²/kg)", B = "LDMC (mg/g)",
                                      C = "LT (µm)", D = "LA (mm²)", E = "WD (g/cm³)") ) )  +
  ylab(NULL) +
  theme(strip.background = element_blank(),
        strip.placement = "outside") +
  xlab("Guild") + scale_fill_discrete(guide = "none")
ggsave(g, filename = "figures/Figure_3.tiff", device = "tiff", dpi = 600, bg = "white", width = 10, height = 4, compression = "lzw")
```
...
R2 <- lapply(fits, mcmc_intervals_data, regex_pars = "R2") %>% 
  ...
  mutate(Variable = "Canopy height", Significance = "significant") %>% 
  mutate(R2 = paste0("R²=", round(R2, 2))) %>% 
   mutate(label = paste(Trait, "-", Guild)) %>% 
  mutate(label = recode(label,
                        "LA - Canopy" = "A LA - Canopy",
                        "LA - Understory" = "B LA - Understory",
                        "LDMC - Canopy" = "C LDMC - Canopy",
                        "LDMC - Understory" = "D LDMC - Understory",
                        "LT - Canopy" = "E LT - Canopy",
                        "LT - Understory" = "F LT - Understory",
                        "SLA - Canopy" = "G SLA - Canopy",
                        "SLA - Understory" = "H SLA - Understory",
                        "WD - Canopy" = "I WD - Canopy",
                        "WD - Understory" = "J WD - Understory"
                        )) 
t <- lapply(fits, mcmc_intervals_data, regex_pars = "beta") %>% 
  bind_rows(.id = "model") %>% 
  separate(model, c("Guild", "Trait"), "_") %>% 
  ...
  mutate(Significance = ifelse(m > 0 & ll > 0, "significant", Significance)) %>% 
  mutate(Significance = ifelse(m < 0 & hh < 0, "significant", Significance)) %>% 
  mutate(Var2 = ifelse(Significance == "significant", as.character(Variable), "A"))
...
g <- t %>% 
  ...
  geom_point(aes(y = m, fill = Var2), shape = 21, size = 3) +
  ...
  scale_color_manual(guide = "none", values = c("grey", RColorBrewer::brewer.pal(5, "Set1"))) +
  scale_fill_manual(guide = "none", values = c("grey", RColorBrewer::brewer.pal(5, "Set1"))) +
  theme(legend.position = "bottom", strip.text.x = element_text(hjust = -0.01))
ggsave(g, filename = "figures/Figure_4.tiff", device = "tiff", dpi = 600, bg = "white", width = 10, height = 4, compression = "lzw")
```
