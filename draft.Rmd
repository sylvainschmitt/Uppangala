...
colnames(DEM_df) <- c("Elevation", "x", "y")
DEM_plot <- ggplot() +  
  geom_tile(data=DEM_df, aes(x=x, y=y, fill=Elevation), alpha=0.8) + 
  scale_fill_distiller(palette = "Spectral") +
  coord_equal() +
  cowplot::theme_map()
DCM_spdf <- as(DCM, "SpatialPixelsDataFrame")
DCM_df <- as.data.frame(DCM_spdf)
colnames(DCM_df) <- c("Height", "x", "y")
DCM_plot <- ggplot() +  
  geom_tile(data=DCM_df, aes(x=x, y=y, fill=Height), alpha=0.8) + 
  coord_equal() +
  cowplot::theme_map() +
  scale_fill_distiller(palette = "Spectral")
ggsave(cowplot::plot_grid(DEM_plot, DCM_plot, labels = c("a) Elevation (m)", "b) Canopy height (m)")), 
       file = "Plot.png",
       path = './data/',
