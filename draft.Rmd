...
$$SES = \frac{(I_{obs} - I_{null})}{\sigma {null}} ~~ (3)$$ 
where $I{obs}$ was the observed metric and $I_{null}$ and $\sigma {null}$ were the mean and standard deviation of the corresponding null distribution. The influence of environment on SES values was investigated by linear regression with biotic and abiotic factors ($SES{trait} = Slope + Curvature + Wetness + SW + Canopy$). Analyses were conducted on the five functional traits.
