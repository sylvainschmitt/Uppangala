...
DCM <- crop(DCM, buffer(limits, 30))
DEM <- crop(DEM, buffer(limits, 30))
...
ggplot() +  
  geom_tile(data=DCM, aes(x=x, y=y, fill=Height)) +
  geom_contour(data=DEM, aes(x=x, y=y, z=Elevation),
               colour = "black", alpha = 0.6, breaks = seq(300, 500, by = 25)) +
  directlabels::geom_dl(data=DEM, aes(x=x, y=y, z=Elevation, label = paste0(..level.., "m")),
                        method = list('last.bumpup', cex = 0.8, hjust = 1),
  ...
  ggsave(file = "Plot.png",
         path = './data/',
         width = 150, height = 200, unit = 'mm',
         dpi = 300)
rm(DEM, DCM, limits, contour, DEM_df, DCM_df, DEM_spdf, DCM_spdf, DCM_plot, DEM_plot)
```
...
```{r PCA,  fig.cap='Principal Component Analysis (PCA) of species mean trait values. Marginal plots represent the density distribution of species on the two principal component axis for each strata. See table \\@ref(tab:Traits) for abbreviation.'}
...
pca.plot$data %>%
  ...
  melt(id.vars = "strata", variable.name = "spectrum") %>% 
  group_by(spectrum) %>% 
  do(anova = aov(value ~ strata, data = .)) %>% 
  broom::tidy(anova)
```

```{r positionTest, eval=F}
data <- pca.plot$data %>% 
  select(Comp.1, Comp.2, strata) %>% 
  rename(WES = Comp.1, LES = Comp.2)
test <- array(NA, c(2,2,n+1))
test[,,1] <- data %>% 
  group_by(strata) %>% 
  summarise_all(mean) %>% 
  select(-strata) %>% 
...
data %>% 
  group_by(strata) %>% 
  summarise_all(mean) %>% 
  cbind(apply(test, 1:2, function(x){
    p <- rank(x)[1]/(n+1)
    if(p > 0.5)
      p <- 1 - p
    return(p)
  })) %>% 
  rename(WES.pval = `1`, LES.pval = `2`)
```
...
```{r GMboxplot, fig.cap="Boxplot of guilds means (GM) among communties for each traits (for canopy (C) and understorey (U) guilds). See table \\@ref(tab:Traits) for abbreviations.", fig.height=4}
L %>%
  melt(id.var = "com",
       variable.name = "SpCode",
       value.name = "abundance") %>% 
  ...
  melt(id.vars = "strata", 
       variable.name = "trait",
       value.name = "GM") %>% 
  ggplot(aes(x = strata, y = GM, fill = strata)) +
  geom_boxplot() +
  facet_wrap(~trait, scales = "free", nrow = 2) +
  xlab("Guild") + scale_fill_discrete(guide = "none") +
...
```{r pcaBRIDGE, fig.cap='Principal Component Analysis (PCA) of species mean trait values for Amazonian species (BRIDGE dataset). Marginal plots represent the density distribution of species on the two principal component axis for each strata.'}
dlim = 0.35
BRIDGE <- readr::read_tsv("./data/species_TROLL_Nino.txt") %>% 
  mutate(strata = ifelse(dmax > dlim, "C", "U"))
...
* Canopy species WES vs LES: `r Variance$LES$C` vs `r Variance$WES$C`, Levene's test p-value=`r Leven_test$C`
* Understorey species LES vs WES: `r Variance$LES$U` vs `r Variance$WES$U`, Levene's test p-value=`r Leven_test$U`

## S4 : Dawkins effect on traits
...
#   left_join(Q %>% 
#               select(SpCode, strata)) %>% 
#   filter(!is.na(strata)) %>% 
#   group_by(com, strata) %>% 
#   summarise(species = length(unique(SpCode)),
#             individuals = sum(individuals)) %>% 
#   ungroup() %>% 
#   # group_by(strata) %>% 
#   summarise(species = mean(species), individuals = mean(individuals))
# genPFT() %>%
#   mutate(CE = gsub('([[:alpha:]])', "", CE)) %>%
#   filter(!is.na(CE)) %>%
#   mutate(LA = log(LA)) %>%
#   mutate(SLA = log(SLA)) %>%
#   select(CE, Sp, WD, LT, LA, LDMC, LTD, SLA) %>%
#   reshape2::melt(id.vars = c("CE", "Sp"), variable.name = "Trait") %>%
#   group_by(Trait) %>%
#   do(anova = TukeyHSD(aov(lm(value ~ CE, data = .)))) %>%
#   tidy(anova) %>%
...
genPFT() %>% 
  mutate(CE = gsub('([[:alpha:]])', "", CE)) %>% 
  filter(!is.na(CE)) %>% 
  select(CE, WD, LT, LA, LDMC, LTD, SLA) %>% 
  mutate(LA = log(LA)) %>% 
  mutate(SLA = log(SLA)) %>% 
  reshape2::melt(id.vars = "CE", variable.name = "Trait") %>% 
  ggplot(aes(CE, value, fill = CE)) +
  geom_boxplot() +
  facet_wrap(~Trait, scales = "free") +
  ggpubr::stat_compare_means(comparisons = my_comparisons, aes(label = ..p.signif..)) +
...
genPFT() %>%
  mutate(CE = as.numeric(gsub('([[:alpha:]])', "", CE))) %>%
  filter(CE != 5) %>% 
  filter(!is.na(CE)) %>%
  ggplot(aes(CE, WD, group = NA)) +
  geom_boxplot(aes(group = CE), col = "lightgrey") +
  geom_point(aes(col = Sp), alpha = 0.3) +
