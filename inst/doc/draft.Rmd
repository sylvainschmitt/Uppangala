...
```{r CWM}
# Observed CWMs
CWM <- lapply(L, function(l){lapply(as.list(c('LES', 'SLA', 'WES', 'WD', 'LDMC', 'LA', 'LT')), function(v){apply(l, 1, function(x){weighted.mean(Q[,v], x, na.rm=T)})})})
CWM <- lapply(CWM, function(x){names(x) <- c('LES', 'SLA', 'WES', 'WD', 'LDMC', 'LA', 'LT') ; return(x)})
## Regressions
...
stopCluster(cl)
rm(cl)
nCWM <- lapply(nCWM, function(y){lapply(y,function(x){names(x) <- c('LES', 'SLA', 'WES', 'WD', 'LDMC', 'LA', 'LT') ; return(x)})})
## Regressions
