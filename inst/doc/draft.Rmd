...
We selected two biotic factors: canopy height and basal area. Canopy height was calculated from a digital canopy model built with the 2013 LIDAR campaign on Uppangala plots with resolution $1*1 m^2$. Basal area was calculated from 2013 girth inventory of Uppangala plot on stems with a diameter above 10 centimeters at breast height ($DBH>10~cm$). 
...
### Local functionnal composition of communities
...
CVNND for each trait $t$ was calculated as nearest neigbor distances (NND) variance, $\sigma _{NND}$, of the $i$ individuals in each community, divided by its mean, $\mu _{NND}$; and NND for one individual $i$ is calculated as the minimal distance $d$ between the $j$ other individuals of the community :
$$CVNND = \frac{\sigma _{NDD}}{\mu _{NDD}} ~ with ~ NND_{i} = min(d_{i,j})_{i \neq j} ~ (3)$$
For both CWV and CVNND, we calculated the standardized effect size (SES) as: 
...
### Functionnal and phylogenetic composition between communities
...
$\beta st$ and $\pi st$ metric proposed by @hardy_characterizing_2007 were used to investigate variation of phylogenetic composition between community pairs. In parrallel variation of functional composition between community pairs was assessed by $U st$ and $\tau st$ metric. Environmental distances between community pairs was also calculated as distances between communities on the plan composed by the two first axes of the principal component analysis of environmental variables (see suppplementary materials S1). Finally spatial distances between community pairs was also assessed. We used Mantel test to sudy the link between functionnal and phylogenetic pairwise compositions with hardy's metrics and environmental distances between community pairs. A second mantel test was realized to assess those pairwise links between environment and functionnal and phylogenetic structure knowing spatial structure with the spatial distances between community pairs matrix.

Observed mantel correlations for phylogenetic composition were compared to 999 obtained with the null model *"1s"*" suggested by @hardy_testing_2008 randomizing phylogenetic tree tips. Observed mantel correlations for functionnal composition were compared to NM1, randomizing traits among species.
...
Table 7: Summary of Moran’s I statistic and p-values for phylogenetic signal of functional traits. _See table 1 for abbreviations. The number of asterisk indicate the p-value threshold ( * p-value<0.05, ** p-value <0.01, *** p-value<0.001)._
...
Table 8: Summary of Mantel test between floristic composition, environment and spatial distances between communities. _$Ist$  was used to investigate variation of floristic composition between communities. Observed values were compared to a null model randomizing individuals among subplots while preserving the regional abundance of species, and the number of individuals per community. E is the mantel test of the metric with the environmental distance matrix issued from PCA two first axes. S|E is the mantel partial test of the spatial distance matrix knowing the environment matrix. The number of asterisk indicate the p-value threshold ( * p-value<0.05, ** p-value <0.01, *** p-value<0.001)._
...
spafun.res <- data.frame(mapply(function(x,y){
  mapply(function(r,s){
    mapply(function(p,q){
      paste(format(p, scientific = T, digits = 3), 
            Uppangala::stars(min(rank(c(p,q))[1]/(n+1), 1-rank(c(p,q))[1]/(n+1)), ns = '   '))
    }, p = r, q = s, SIMPLIFY = F)
  }, r = x, s= y)
}, x = spafun.mantel, y = nspafun.mantel))
spafun.res <- cbind(data.frame('Mantel test' = c('E', ' ', 'S|E', ' '), 'Metric' = rep(c('$U_{st}$', '$\\tau _{st}$'), 2)), spafun.res)
