---
title: 'Uppangala tree functionnal: RLQ draft'
...
axis (3)
axis (4)
# arrows (rep(0,nrow(R$co)),rep(0,nrow(R$co)),R$co[,1],R$co[,2],lwd=1, col = 'blue')
# text(R$co, pos = c(4, 2, 4, 2, 3), rownames(R$co),cex=0.9,col='blue')
...
Lm <- Lm / rowSums(Lm)
Lm.l$`basal area` = Lm
RLQm.l$`-WD` <- list(Rm.l = rep(list(Rm),3), Lm.l = Lm.l, Qm.l = rep(list(Qm),3))
# str(RLQm.l,3)

# FQR
fqr <- lapply(RLQm.l, function(x){mapply(function(r,l,q){fourthcorner(r, l, q, modeltype = 6, p.adjust.method.G = "none", p.adjust.method.D = "none")}, r = x$Rm.l, l = x$Lm.l, q = x$Qm.l, SIMPLIFY = F)})
names(fqr$full) <- names(Lm.l) ; names(fqr$`-WD`) <- names(Lm.l)

# Result
# par(mfrow = c(2,3))
# lapply(fqr, function(x){lapply(x, plot)})
fqr.df.l <- mapply(function(y,z){
  fqr.df <- with(y$tabG, cbind(data.frame(t(matrix(unlist(strsplit(names, ' / ')), nrow = 2)), stringsAsFactors = F), obs, pvalue))
  fqr.wd <- with(z$tabG, cbind(data.frame(t(matrix(unlist(strsplit(names, ' / ')), nrow = 2)), stringsAsFactors = F), obs, pvalue))
  fqr.df <- rbind(fqr.df, fqr.wd[which(fqr.wd$X2 == 'WD'),])
  names(fqr.df)[1:2] <- c('Factor', 'Trait') 
  fqr.df$res <- paste(round(fqr.df$obs, 3), '/', round(fqr.df$pvalue, 3), unlist(lapply(as.list(fqr.df$pvalue), stars, ns = ' ')))
  fqr.df <- tapply(fqr.df$res, INDEX = list(fqr.df$Trait, fqr.df$Factor), paste, simplify = F)
  return(fqr.df)
  }, y = fqr$`-WD`, z = fqr$full, SIMPLIFY = F)
```
```{r FQR abundance}
kable(t(fqr.df.l$abundance))
```
...
## S1: Fourth corner analysis results in presence-absence and relative basal area

```{r FQR others}
kable(t(fqr.df.l$`presence-absence`))
kable(t(fqr.df.l$`basal area`))
```
