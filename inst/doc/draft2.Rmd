...
GWM_ses %>% 
  ...
  summarise_all(mean, na.rm = T) %>% 
   kable(caption = 'Mean of the standardized effect size (SES) with null model NM1 for each trait of GWMs, for canopy (C) and understorey (U) guilds, in abundance. See table \\@ref(tab:Traits) for abbreviations. Bold and number of asterisk indicates the significance threshold ( * p-value<0.05, ** p-value <0.01, *** p-value<0.001).')
```
...
GWM_ses %>% 
  ...
  group_by(strata) %>% 
  summarise_all(mean, na.rm = T) %>% 
   kable(caption = 'Mean of the standardized effect size (SES) with null model NM3 for each trait of GWVs, for canopy (C) and understorey (U) guilds, in abundance. See table \\@ref(tab:Traits) for abbreviations. Bold and number of asterisk indicates the significance threshold ( * p-value<0.05, ** p-value <0.01, *** p-value<0.001).')
```
