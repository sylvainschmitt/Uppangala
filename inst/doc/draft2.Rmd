...
We first tested functional compostion differentiation between guilds within each community with a wilcoxson test, to check if the local functional composition from guilds differed. Inside each community we tested the difference between all individuals of each guilds, and condidered the difference to be significative with a p-value inferior to 5%. We then summarized communities significativelly differing in their functional traits between understorey and canpoy guilds for each trait.

Then we devised a null model approach to test for any significant changes in functional composition in response to an environmental gradient. We thus wanted to explore if the local functional composition reflect the influence of abiotic and biotic drivers of community dynamics within guilds. We consequently used subsequent hypothesis:

* ___H1__ Drivers of environmental filtering on local functional composition differ between canopy and understorey tree species._ 
* ___H2__ Strength of drivers of evironmental filtering differ between canopy and understorey tree species._ 
    
To accept or reject hypothesis we used two null models :

* __NM1__ randomized trait values among species in the whole plot for each guild (permuting raw in `Trait*Species` matrices of both guilds) [@bernard-verdier_community_2012]. Consequently, NM1 breaks the link between environement and local functional composition while keeping guilds and species spatial structure, thus testing for hypothesis __H1__.
* __NM2__  randomized abundances among species within each guild but kept the observed list of species and trait values unchanged inside guilds [@bernard-verdier_community_2012]. Consequently NM2, breaks the link between environment and species abundances while keeping guilds, community, and species spatial structure, thus testing for hypothesis __H2__.
...
$$SES = \frac{(I_{obs} - I_{null})}{\sigma _{null}} ~~ (3)$$
...
Except for Specific Leaf Area (SLA), most of the traits were not significantly different between guilds in the majority of communities (only 20 to 41% significative for WD, LDMC, LA and LT against 79% for SLA, see figure \@ref(fig:percsign) ). But regarding guilds weighted means (GWM), understorey and canopy guilds strongly diverged among communities (see figure \@ref(fig:CWMboxplot) ). Canopy guilds have higher SLA, WD and LA but lower LDMC and LT than understorey guilds. In other words, canopy guilds have larger, thinner and lighter leaves with increased SLA and denser woods, whereas understorey guilds have smaller, thicker, and more dense leaves with lower SLA and lighter woods.
...
L %>% 
  ...
  left_join(Q) %>% 
  select(- SpCode, -LES, -WES) %>% 
  melt(id.vars = c("community", "strata"),
       variable.name = "trait") %>% 
  group_by(community, trait) %>% 
  do(wilcoxon = try(wilcox.test(value~strata, data=.)$p.value)) %>% 
  filter(!inherits(wilcoxon, "try-error")) %>% 
  mutate(wilcoxon = unlist(wilcoxon)) %>% 
  group_by(trait) %>% 
  mutate(test_0.5 = wilcoxon < 0.05) %>%
  dplyr::summarise(com_sign = sum(test_0.5)/n()*100) %>%
  ggplot(aes(trait, com_sign)) +
  geom_bar(stat = "identity") +
  xlab("Functional trait") +
  ylab("Percentage of significant communities")
```

```{r GWMH1a, fig.cap="Number of communities significantly (p-value<0.05) different from null expectations with null model NM1 for each trait of GWMs, for canopy (C) and understorey (U) guilds, in abundance. See table \\@ref(tab:Traits) for abbreviations.", fig.height=4, include = F}
GWM <- L %>%
  t() %>%
  data.frame() %>%
  mutate(SpCode = row.names(.)) %>%
...
# rm(cl, nQ)
# save(nGWM, file = file.path(path, 'nGWM_NM1.Rdata'))
load(file.path(path, 'nGWM_NM1.Rdata'))
GWM_pval <- GWM
GWM_pval[3:9] <- apply(
  array(c(as.matrix(GWM[3:9]), nGWM), c(dim(GWM[3:9]), n+1)), 
  1:2, function(x) rank(x)[1]/(n+1))
```

```{r CWMboxplot, fig.cap="Boxplot of guilds weighted means (GWM) among communties for each traits (for canopy (C) and understorey (U) guilds). See table \\@ref(tab:Traits) for abbreviations.", fig.height=4, include = T}
GWM %>% 
...
```{r GWVH1b, fig.cap="Number of communities significantly (p-value<0.05) different from null expectations with null model NM3 for each trait of GWVs, for canopy (C) and understorey (U) guilds, in abundance. See table \\@ref(tab:Traits) for abbreviations.", fig.height=4, include = F}
GWV <- L %>%
  t() %>%
  data.frame() %>%
  filter(sum(.) > 2) %>%
...
# rm(cl, nL)
# save(nGWV, file = file.path(path, 'nGWV_NM3.Rdata'))
load(file.path(path, 'nGWV_NM3.Rdata'))
GWV_pval <- GWV
GWV_pval[3:9] <- apply(
  array(c(as.matrix(GWV[3:9]), nGWV), c(dim(GWV[3:9]), n+1)), 
  1:2, function(x) rank(x)[1]/(n+1))
GWV_pval %>% 
  select(-community, -LES, -WES) %>% 
  group_by(strata) %>% 
  mutate_all(function(x) findInterval(x, c(0.05, 0.95)) != 1) %>% 
  melt(id.vars = "strata", variable.name = "trait") %>% 
  filter(value == TRUE) %>% 
  ggplot(aes(x = factor(trait), 
             fill = factor(strata))) +
...
```{r CWVboxplot, fig.cap="Caption", fig.height=8, include = F}
GWV %>% 
  select(-community, -LES, -WES) %>% 
  melt(id.vars = "strata", 
       variable.name = "trait",
       value.name = "GWV") %>% 
  ggplot(aes(x = strata, y = GWV, fill = strata)) +
  geom_boxplot() +
  facet_wrap(~trait, scales = "free") +
  ggpubr::stat_compare_means(aes(label = paste("Wilcoxon, p",
                                               ..p.format..,
                                               ..p.signif..)))
```
...
The guild weighted trait means (GWM) varied weakly but significantly along the environmental gradients for both canopy (C) and understorey (U) guilds (see table \@ref(tab:GWMH2a)). Functional traits of canopy and understorey tree species were both influenced by curvature, but not for the same traits (see figure \@ref(fig:WD)), and two major differences were observed between the two guilds. The effect of south-west orientation on functional composition was observed only for canopy tree species, while the effect of canopy height and wetness affected mainly understorey guild composition.
...
For understorey tree species, SLA significantly decreased and and leaf thickness significantly increased with curvature. SLA significantly increased with canopy height, while leaf thickness decreased with canopy height. Finally, SLA and wood density significantly decreased and and leaf area and thickness significantly increased with wetness.
...
#               list(row.names(GWM), names(GWM[3:9]), 1:n))
# rm(cl, nQ)
# save(nGWM, file = file.path(path, 'nGWM_NM2.Rdata'))
load(file.path(path, 'nGWM_NM2.Rdata'))
GWM_ses <- GWM
...
GWM_ses %>% 
  ...
  left_join( # Inclusion of R2 but should be split
    GWM_ses %>% 
      ...
      select(-community) %>% 
      group_by(strata, trait) %>% 
      do(glance(step(lm(SES ~ Slope + Curvature + Wetness + SW + Canopy + BA, .), 
                     direction = 'both', trace = 0))) %>% 
      select(r.squared) %>% 
...
We found significant relationships between the standardized effect size (SES) of GWVs and the environmental predictors (see table \@ref(tab:GWVH2b)). Curvature increased LDMC variance in canopy. Wetness decreased variance of SLA and wood density in canopy and leaf area in understorey, but increased variance of wood density in understorey. Sout-West orientation results in decreased variance of density of leaf in understorey. Finally, Canopy height diminished variance of leaf density in canopy whereas basal aea increased variance of SLA in both strata.
...
We did not found significant differentiation of functional hypervolumes between canopy and understorey species. Still the two hypervolumes were only half overlaped with a p-value of $p = 0.118$ (see [S2: Hypervolumes of functional guilds]).

Furthermore, we analyzed functional composition of $30*30m^2$ guilds from canopy and understorey, as well as the variation of this composition with environment across a 10-ha forest plot. We first evidenced guild differenciation in 15 to 80% of communities depending on functional traits (see figure \@ref(fig:percsign)). We are above a 5% type one error, and thus we conclude to a global functional differentiation between guilds within communities. But the differentiation differ between functional traits. Specific Leaf Area (SLA) was differentiated aong guilds in the majority of communities whereas Leaf Thickness (LT) was the less marked. This different levels of differentiation among guilds between functional traits could explain the lack of significance with the hypervolume overlap test when considering all functional traits together. Anyway, distribution of functional composition among communities shows a strong differentiation between the two guilds for each trait (see figure \@ref(fig:CWMboxplot)). Canopy guilds have larger, thinner and lighter leaves with increased SLA and denser woods, whereas understorey guilds have smaller, thicker, and more dense leaves with lower SLA and lighter woods. 

In addition, we foud that the variations in trait composition along the environmental gradients differ between canopy and understorey species, with a marked variation in traits associated to the leaf economic spectrum (LES) for understorey species and to the wood economics scpectrum (WES) for canopy species. In the former case, the curvature, wetness index, and the canopy height had a significant effect on the LES-based composition of understorey tree species, whereas southwest orientation and curvature both had a significant effect on the WES-based composition of canopy tree species (table \@ref(tab:GWMH2a) and \@ref(tab:GWVH2b)).
...
```{r hypervfig, fig.cap='Functional hypervolume overlap of canopy and understorey species.', fig.height=4}
hypervolume_fig
```
