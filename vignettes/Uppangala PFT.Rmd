...
# cv$sp <- sd
# mean(aggregate(PFT_ind, by = list(PFT_ind$SP), function(x){sd(x, na.rm = T) / mean(x, na.rm = T)})$Thick, na.rm = T)
# sd(PFT_ind$Thick, na.rm = T) / mean(PFT_ind$Thick, na.rm = T)
...
```{r CWM abundance, message=FALSE, warning=FALSE}
# CWM <- aggregate(Trees, by = list(Trees$com), mean, na.rm = T)
# names(CWM)[1] <- 'id'
# CWM <- CWM[which(names(CWM) %in% c('id', 'Thick', 'LA', 'LDMC', 'LTD', 'SLA', 'WD', 'PCA1', 'PCA2', 'PCA3'))]
# com <- merge(com, CWM)
# rm(CWM)
# summary(com)
# plot(stack(com)[[which(names(com) %in% c('Thick', 'LA', 'LDMC', 'LTD', 'SLA', 'WD', 'PCA1', 'PCA2', 'PCA3'))]])
# plot(gstat::variogram(WD~1, as(com, 'SpatialPointsDataFrame')))
```
```{r CWM binary, message=FALSE, warning=FALSE}
# Trees_bin <- Trees[-which(duplicated(cbind(Trees$SpCode, Trees$com))),]
...
```
```{r CWM basal area, message=FALSE, warning=FALSE}
library(plyr)
TreesBA <- Trees[-which(is.na(Trees$`2013_girth`)),]
...
LM <- lapply(LM, function(x){step(x, direction = 'both', trace = 0)})
lapply(LM, summary)
```
...
Correlations with Speraman

## Null models

```{r Model, echo=FALSE, fig.height=9, fig.width=9, message=FALSE, warning=FALSE} 
model <- null_model(formulas = lapply(LM, function(x){as.character(x$call)[2]}), 
                    ba = T, test = 'lm deviance',
                    PFT_sp = PFT_sp, Trees = TreesBA, com = com, SpCode = SpCode,
                    n = 100, plot = F, count = F, time = T)
par(mfrow = c(2,4))
for(i in 1:8){
  plot_model(model, index = i)
}
knitr::kable(regression_table(LM, model, reg))
```
